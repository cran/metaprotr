#' filter_shared
#'
#' Keeps the elements from a "spectral_count_object" that are common to all 
...
#' 
#' @return A list defined as "spectral_count_object" in which 
  ...
  if(length(spectral_count_object) == 4){
    if(spectral_count_object[[4]] == "spectral_count_object"){
      ...
      if(metadata_feature %in% colnames(metadata) == TRUE){
        ...
        #--------- Same format of Spectral counts, data was only filtered
